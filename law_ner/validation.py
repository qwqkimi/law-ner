...
from law_ner.utils.bert_embedding import bert_embedding,bert_embedding_for_str
...
def bert_ner_val(model_name,dict_path=dict_path,summary=True):
    ...
    predict_text='因涉嫌犯危险驾驶罪,于2014年2月4日被北京市延庆县公安局羁押,次日被刑事拘留,现羁押于北京市延庆县看守所。'
    s=bert_embedding_for_str(predict_text)
    
    raw = model.predict(s)[0]
    result = [np.argmax(row) for row in raw]
    result_tags = [chunk_tags[i] for i in result]
...
def bert_ner_prf(model_name,train_path=train_path,test_path=test_path,dict_path=dict_path,summary=True):

    model = BertNER()
    model.build_model(batch_shape, lstm_units, 3)
    model.compile()
    model.load_weight(models_path,model_name=model_name)
...
if __name__=='__main__':
    ...
    # ner_val(model_name)
    ner_prf(model_name)
    # bert_ner_prf(model_name='bert_ner_train_test')