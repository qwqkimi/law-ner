...
from law_ner.keras_contrib_crf.crf_accuracies import crf_marginal_accuracy
from law_ner.keras_contrib_crf.crf_accuracies import crf_viterbi_accuracy
from law_ner.keras_contrib_crf.keras_utils import to_tuple
...
class CRF(Layer):
    ...
    ```python
        from keras.model_builder import load_model
    ...
    # Masking
        This layer supports masking for input dataset with a variable number
        of timesteps. To introduce masks to your dataset,
    ...
    def recursion(self, input_energy, mask=None, go_backwards=False,
        ...
        if mask is not None:
            mask_tensor = tf.dtypes.cast(mask, tf.float32)
            # mask_tensor = tf.dtypes.cast(mask, tf.float32) if tf_version() == 2 else tf.to_float(mask)
            mask2 = K.cast(K.concatenate([mask_tensor, K.zeros_like(mask[:, :1])], axis=1),
                           K.floatx())
